...
public class Invisible_Hand implements Steppable{
    ...
    public void step(SimState state) {
        ...
        if(state.schedule.getSteps()>1){
            ...
            dyn=1;//1 an thelo na allazoun dinamika oi times gia ta peiramata
            //allagi metavliton ana 4 xronia
            if(state.schedule.getSteps()%8==0  && dyn==1){
                /*Random qr = new Random();
                double qr1=qr.nextDouble();
                if(Institutions.officials_behavior<10 && qr1<0.5)
                Institutions.officials_behavior++;
                if(Natural_Environment.accesibility<10 && qr1<0.3)
                Natural_Environment.accesibility++;
                if( Natural_Environment.techdevelop<10 && qr1<0.2)
                Natural_Environment.techdevelop++;*/

                /*if(Institutions.officials_behavior<10)
                Institutions.officials_behavior++;
                if(Natural_Environment.accesibility<10 )
                Natural_Environment.accesibility++;
                if( Natural_Environment.techdevelop<10 )
                Natural_Environment.techdevelop++;*/
                ...
                if(Institutions.officials_behavior>0 )
                    Institutions.officials_behavior--;
                /*if(Institutions.officials_behavior>0 && qr1>=0.5)
                Institutions.officials_behavior--;*/
            ...
            for(int j=0;j<Social_Norms.agents.size();j++){
                ...
                if(ran<pososto_apoliseon){
                    if(Social_Norms.agents.get(j).wage!=0 && Social_Norms.agents.get(j).work.size()!=0){//exei proigoumenei doulia pou prepei na afisei(paraitisi)
                    ...
                    Social_Norms.agents.get(j).work.clear();//!!!!!!prosoxi mipos den svinontai
     ...
     //na kano apoliseis me pososto oso einai i anergia DONE
    //na prostheto nees etaireie simfona me to an mporoun na leitourgisoun! DONE
    //na allazo tis metavlite simperifora institouton, accesibility ktl. DONE
